...
bool GameRule::shipAddCorrect(int shipSize, Coordinates coordinates, Direction direction, std::unique_ptr<Board> board) {
    if (board->shipsLeftToSet.find(shipSize) == board->shipsLeftToSet.end() ||
        board->shipsLeftToSet.find(shipSize)->second <= 0 || !insideBoard(coordinates, board->size)) {
    ...
    while (fieldsToSet > 0) {
        if (!insideBoard(coordinates, board->size) || board->shipFieldValue(coordinates)) {
        ...
        for (Direction dir: Coordinates::getListOfAllDirections()) {
            ...
            if (insideBoard(appliedDirectionCoordinates, board->size) &&
...
bool GameRule::shipInThisDirectionUncovered(Coordinates coordinates, std::unique_ptr<Board> board,
                                            Direction direction) {
    coordinates = Coordinates::applyDirectionChange(coordinates, direction);
    while (insideBoard(coordinates, board->size) && board->shipFieldValue(coordinates)) {
...
bool GameRule::insideBoard(Coordinates coordinates, int boardSize) {
...
int GameRule::getNumberOfShipsOfThisSizeWhenThisBoardSize(int shipSize, int boardSize) {
    switch (shipSize) {
        case 2:
